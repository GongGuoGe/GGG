...
@implementation CountSqlService
...
//插入数据
-(BOOL) insertTestList:(sqlCountList *)insertList {
...
- (NSMutableArray*)getTestList{
	...
	if ([self openDB]) {
		...
		//sql语句
		char *sql = "SELECT ID, countType,CountNum FROM CountTable";
...
//更新数据
-(BOOL) updateTestList:(sqlCountList *)updateList{
	...
	if ([self openDB]) {
		...
		if (success != SQLITE_OK) {
			NSLog(@"Error: failed to update:CountTable");
			sqlite3_close(_database);
			return NO;
		}
...
//删除数据
- (BOOL) deleteTestList:(sqlCountList *)deletList{
	if ([self openDB]) {
		
		sqlite3_stmt *statement;
		//组织SQL语句
		static char *sql = "delete from CountTable  where ID = ?";
		//将SQL语句放入sqlite3_stmt中
		int success = sqlite3_prepare_v2(_database, sql, -1, &statement, NULL);
		if (success != SQLITE_OK) {
			NSLog(@"Error: failed to delete:CountTable");
			sqlite3_close(_database);
			return NO;
		}
