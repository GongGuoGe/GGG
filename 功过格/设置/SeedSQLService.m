...
- (BOOL) createTestList:(sqlite3*)db {
	...
	//这句是大家熟悉的SQL语句
	char *sql = "create table if not exists SeedTable(ID INTEGER PRIMARY KEY AUTOINCREMENT, seedName text)";
...
//插入数据
-(BOOL) insertTestList:(sqlSeedList *)insertList {
	...
	if ([self openDB]) {
		...
		static char *sql = "INSERT INTO SeedTable(seedName) VALUES(?)";
	...
	if ([self openDB]) {
		...
		if (sqlite3_prepare_v2(_database, sql, -1, &statement, NULL) != SQLITE_OK) {
			...
			while (sqlite3_step(statement) == SQLITE_ROW) {
				sqlSeedList* sqlList = [[sqlSeedList alloc] init] ;
...
//更新数据
-(BOOL) updateTestList:(sqlSeedList *)updateList{
...
//删除数据
- (BOOL) deleteTestList:(sqlSeedList *)deletList{
	if ([self openDB]) {
...
- (NSMutableArray*)searchTestList:(int)searchID{
	...
	if ([self openDB]) {
		...
		sqlite3_stmt *statement = nil;
		//sql语句
		char *sql = "SELECT testID, testValue FROM CountTable WHERE testID = ?";
